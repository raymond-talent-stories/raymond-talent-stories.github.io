...
```
`React`支持`setTimeout`以及`MessageChannel`实现时间切片，在现代浏览器中是通过`MessageChannel`实现的，基于宏任务可以通过异步方式处理任务队列而不用担心宏任务的执行影响浏览器的渲染过程，而每个时间切片的长度在`React`中会控制在5ms左右。让我们尝试使用`MessageChannel`实现一个类似的时间切片机器。
<iframe style="height: 600px; border: none;" src="/examples/time-slicing/">